...
leave that field unchanged rather than fabricate.`;
  ...
  function buildAcceptedPatch() {
    // Non-destructive: emit a dotted-path PATCH of accepted changes only — NOT a
    // full file. The walk already produced per-path changes (arrays wholesale),
    // which is exactly the patch format apply_studio_patch.py consumes
    // (SPEC studio-data-safety §4.1). A full file would revert any field written
    // upstream after this fiche was loaded.
    const patch = {};
    for (const c of changes) {
      if (rejected.has(c.path)) continue;
      if (c.path === '(root)') continue;  // defensive; shouldn't occur
      patch[c.path] = c.newValue;
    }
    // If the user dropped a hero image, point at it (overrides any walked value).
    if (imageBlob) {
      patch['hero_image'] = `/${original.slug}-hero.${imageExt}`;
      patch['hero_credit'] = null;
    }
    return {
      slug: original.slug,
      source: 'studio-enricher',
  ...
  async function exportZip() {
    ...
    const zip = new JSZip();
    const patchDoc = buildAcceptedPatch();
    const slug = patchDoc.slug;
    ...
    const readme = [
      ...
      `Changes accepted: ${changes.length - rejected.size} of ${changes.length}`,
      `Image included: ${imageBlob ? 'yes (' + slug + '-hero.' + imageExt + ')' : 'no'}`,
      ``,
      `This is a PATCH (changed keys only), not a full fiche. It only ever`,
      `replaces the paths it lists; everything else in the live JSON is left`,
      `byte-identical (SPEC studio-data-safety).`,
      ``,
      `## To integrate`,
      ``,
      `1. git pull --ff-only   (so the merge target is fresh live JSON)`,
      ...
      `        (add --dry-run first to preview; never cp a full file over Json/)`,
      `4. Run: python3 scripts/build_all.py --no-site   (re-render + gates)`,
      `5. Commit + push.`,
      ``,
      `## Rejected changes (not in this patch)`,
