---
slug: cascade-du-chinaillon
name: "Cascade du Chinaillon"
category: cascade
category_label: "Cascade"
commune: "Le Grand-Bornand"
postal_code: "74450"
department: "Haute-Savoie"
department_code: "74"
region: "Auvergne-Rhône-Alpes"
country: France
latitude: 45.962517
longitude: 6.439147
geo_verified: false
google_place_id: "ChIJ4xK5PNr2i0cRRtm7GJYEEGA"
canonical_url: https://loisirs74.fr/en/cascade-du-chinaillon
language: en
facet_json: https://loisirs74.fr/api/lieu/cascade-du-chinaillon.json
photo_url: https://upload.wikimedia.org/wikipedia/commons/4/4f/Le_Chinaillon_%40_Pont_des_Romains_%40_Le_Grand-Bornand_%2851027594708%29.jpg
photo_type: real
photo_author: "Cascade du Rouget"
photo_license: "Wikimedia Commons"
photo_source: CC BY-SA
last_updated: 2026-05-14
source: loisirs74.fr
---

# Cascade du Chinaillon

## Facts

- Commune: Le Grand-Bornand, Haute-Savoie (74450)
- GPS: 45.962517, 6.439147
- Category: cascade
- Type: Natural waterfall

## Hours

Not specified

## Prices

Free

## Access (PMR)

Not specified

## Parking

Free (Chinaillon car parks)

## Transport

- Les Faux (Cars Région Haute-Savoie, ~216 m, lines N-ARAVISBUS)
- Le Haut des Frasses (Cars Région Haute-Savoie, ~309 m, lines N-ARAVISBUS)
- Les Nants (Cars Région Haute-Savoie, ~337 m, lines N-ARAVISBUS)
- Source: transport.data.gouv.fr — Licence Etalab 2.0 (2026-06-15)

## Season

April – October (spring for maximum flow)

## Official source

http://www.legrandbornand.com/la-cascade-mysterieuse.html
