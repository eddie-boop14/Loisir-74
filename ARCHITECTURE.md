...
|---|---|---|
| `build-gate.yml` | every push to `main` + PRs | `build_all.py` with gates: status, hygiene (0 scaffolding leaks), full 2352-HTML render, catalog, hubs, **placement gate (protects chez-nous-a-la-plage + chalet-du-tornet hosts)**, **partner-card byte-diff gate**, reachability (0 orphans ×6 langs), byte-stable double-build. **Truth (2026-08-04): the gate and the Netlify deploy are PARALLEL, not chained** — both react to the push independently; a red gate emails + fails the check but does NOT physically stop the deploy (runs #385-388 proved it: Netlify shipped while the badge was red). Red blocks PR merges (if required) and blocks *you*, by contract. Chaining deploys behind the gate (Actions-driven `netlify deploy`) is a known upgrade, deliberately not done casually — the deploy pipeline feeds 800+ AI citations/day and does not get rewired on a whim. |
